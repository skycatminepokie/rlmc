...
from skycatdev.rlmc.java_environment_wrapper import WrappedJavaEnv, WrappedBlockHitResult, java_list_to_array
...
class WrappedSkybridgeEnvironment(WrappedJavaEnv):
    ...
    def obs_to_python(self, java_obs: JavaObject) -> ObsType:
        assert isinstance(java_obs.blocks(), JavaList), f"Expected JavaList, got {type(java_obs.blocks())}"
        agent = java_obs.self()
        return dict[
            "blocks": [WrappedBlockHitResult(hit_result).to_dict(agent.getWorld()) for hit_result in java_obs.blocks()],
            "x": agent.getX(),
            "y": agent.getY(),
            "z": agent.getZ(),
            "yaw": self.java_view.net.minecraft.util.math.MathHelper.wrapDegrees(agent.getYaw()),
            "pitch": self.java_view.net.minecraft.util.math.MathHelper.wrapDegrees(agent.getPitch()),
            ...
            "history": [self.action_to_python(e) for e in java_list_to_array(java_obs.history)]
        ]
    ...
    def action_to_python(self, action: JavaObject) -> ActType:
        ...
        assert isinstance(action_types, JavaList), f"Expected a JavaList, got a(n) {type(action_types)}"
